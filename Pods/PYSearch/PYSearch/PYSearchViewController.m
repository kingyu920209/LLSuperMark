...
- (UILabel *)searchHistoryHeader
{
    if (!_searchHistoryHeader) {
        UILabel *titleLabel = [self setupTitleLabel:[NSBundle py_localizedStringForKey:@"历史搜索"]];
...
- (void)setup
{
    ...
    [cancleButton setTitle:[NSBundle py_localizedStringForKey:@"取消"] forState:UIControlStateNormal];
    [cancleButton setTitleColor:[UIColor py_colorWithHexString:@"04d29d"] forState:(UIControlStateNormal)];
    ...
    UILabel *titleLabel = [self setupTitleLabel:[NSBundle py_localizedStringForKey:@"热门搜搜"]];
...
{
    return self.showSearchHistory && self.searchHistories.count && PYSearchHistoryStyleCell == self.searchHistoryStyle ? (self.searchHistoryTitle.length ? self.searchHistoryTitle : [NSBundle py_localizedStringForKey:@"历史搜索"]) : nil;
}
